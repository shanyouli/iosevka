    ...
    flake-parts.lib.mkFlake { inherit inputs; } (
      ...
      let
        ...
        isTermFont =
          ...
          in
          ("term" == attrs.buildPlans.${pname}.spacing);
        mapTomls =
          dir:
          let
            isTomlFile = n: v: v == "regular" && (lib.hasSuffix ".toml" n) && (n != "private-build-plans.toml");
            getPname =
              f: builtins.head (builtins.attrNames (builtins.fromTOML (builtins.readFile f)).buildPlans);
          in
          (lib.filterAttrs (n: v: v != null) (
            lib.mapAttrs' (
              n: v:
              (
                if (isTomlFile n v) then
                  (
                    let
                      f = dir + "/${n}";
                      pname = getPname f;
      ...
      in
      {
        ...
        perSystem =
          ...
          }:
          {
            ...
            packages.nerd-font-patcher = pkgs.nerd-font-patcher.overrideAttrs (old: {
              src = (pkgs.callPackages ./_sources/generated.nix { }).nerd-font-patcher.src;
              ...
              ci = {
                ...
                program = pkgs.writeTextFile rec {
                  ...
                  text =
                    let
                      attrs = mapTomls ./src;
                      nerdfonts = lib.filterAttrs (_: v: v == true) (
                        lib.mapAttrs' (n: v: lib.nameValuePair n (isTermFont v)) attrs
                      );
                    in
                    ''
                      #!${pkgs.nushell}/bin/nu
                      let packageName = "${
                        lib.escape [ "\"" "\\" ] (builtins.toJSON (builtins.attrNames attrs))
                      }" | from json
                      ...
                      let nerdnames = "${
                        lib.escape [ "\"" "\\" ] (builtins.toJSON (builtins.attrNames nerdfonts))
                      }" | from json
                      for $i in $nerdnames {
                  ...
                  lib.makeScope final.newScope (
                    ...
                    (
                      ...
                      lib.mkMerge [
                        ...
                        (lib.mkIf (isTermFont v) rec {
                          nerd = final.stdenvNoCC.mkDerivation {
                            pname = "${n}-nerd";
                            version = base.version;
                            src = base;
                            nativeBuildInputs = [
                              (prev.nerd-font-patcher.overrideAttrs (old: {
                                src = (prev.callPackages ./_sources/generated.nix { }).nerd-font-patcher.src;
                                nativeBuildInputs = (old.nativeBuildInputs or [ ]) ++ [ prev.p7zip ];
                                unpackPhase = ''
                                  7z x -y $src
                                '';
                              }))
